...
class ClassCache:
    ...
    def __init__(self, n, reps, edge_to_index):
        super().__init__()
        self.n = n
    ...
    def __getitem__(self, item):
        if item not in self.cache:
            self.cache[item] = self._class_id(graph_from_mask(item, self.edge_to_index, self.n))
...
class K222MatroidProblem:
    def __init__(self, n: int):
        ...
        self.edge_to_index = edge_to_index
        self.class_cache = ClassCache(n, reps, edge_to_index)
    ...
    def preprocess_class_cache(self):
        if self.workers is None:
            num_workers = max(1, cpu_count() - 1)

        total = 1 << self.m
        chunk_size = (total + self.workers - 1) // self.workers
    ...
    def _add_monotonicity_constraints(self, workers: int):
        def _monotonicity_worker(args):
            ...
            for mask in range(start, end):
                if (mask - start) % 100_000 == 0:
                    logger.info(f"\rWorker {worker}: {mask}/{end}")

                id_a = self.class_cache[mask]

                for e_idx in range(self.m):
                    if not ((mask >> e_idx) & 1):
                        id_b = self.class_cache[mask | (1 << e_idx)]
        ...
        for worker_id in range(workers):
            ...
            if start_mask < end_mask:
                tasks.append((worker_id, start_mask, end_mask))

        logger.info(f"Generating monotonicity constraints using {workers} workers...")
        with Pool(processes=workers) as pool:
            results = pool.map(_monotonicity_worker, tasks)
...
def add_all_elementary_submodularity_constraints_parallel(
    ...
    logger.info(f"Generating elementary submodularity constraints using {num_workers} workers...")
...
def solve_with_all_elementary_submodularity(args):
    ...
    logger.info("Added elementary submodularity constraints:", added)
