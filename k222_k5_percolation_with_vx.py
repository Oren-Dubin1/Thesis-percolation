...
def check_vx_all_graphs(G: nx.Graph,
                      v: int,
                      x: int,
                      depth: int = 1,
                      base_graphs: Optional[Set] = None
    ...
    connected to at least 3 vertices other than v, and percolating without v.
    Recursively check graphs up to given depth.
    The conjecture is that (v,x) will not be added unless another pair in the initial K_222 is also added.
    :param G: Initial Graph
    :param v:
    :param x:
    :param target_vertex:
    ...
    :return:
    """
    print(f"Entering check_conj_all_graphs: depth={depth}, v={v}, x={x}")
    ...
    for idx, graph in enumerate(base_list, start=1):
        ...
        for H_idx, H in enumerate(graphs, start=1):
            ...
            witnesses = witnesses_for_edge(H, (v,x))
            if len(witnesses) and not H.has_edge(2,3):
                print(f"\n        Counterexample found: edge ({v},{x}) addable by witnesses {witnesses}; printing graph then raising")
                PercolationGraph(H).print_graph()
                raise ValueError
    ...
    print("Recursing to next depth level")
    return check_vx_all_graphs(G, v, x, depth=depth, base_graphs=checked_graphs)
...
if __name__ == "__main__":
    ...
    v = 0
    x = 1
    print(check_vx_all_graphs(G, v, x, depth=4))